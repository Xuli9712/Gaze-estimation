...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=0,
                 ...
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0], feature=True, **kwargs):
        super().__init__()
        ...
        self.new_head_for_gaze_point = nn.Linear(self.num_features, 2)
    ...
    def forward(self, x):
        if self.feature == True:
            ...
            x = self.forward_features(x)
            x = self.new_head_for_gaze_point(x)
            #x = self.head(x)
        return x
