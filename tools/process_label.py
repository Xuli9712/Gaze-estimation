...
def read_screen_mat(screen_path):
    ...
    screen_width = float(mat_data['width_mm'][0][0])*0.1
    screen_height = float(mat_data['height_mm'][0][0])*0.1
    return width_ratio, height_ratio, screen_width, screen_height
...
def process_txt_and_images(src_dir, label, prefix="p"):
    for dir_name in os.listdir(src_dir):
        if dir_name.startswith(prefix):
            ...
            width_ratio, height_ratio, screen_width, screen_height = read_screen_mat(screen_mat)
            ...
            if os.path.isfile(txt_file):
                with open(txt_file, 'r') as file:
                    for line in file:
                        ...
                        gaze_point_cm_center = [round(int(info1[1])*width_ratio, 5)-0.5*screen_width, round(int(info1[2])*height_ratio, 5)]
